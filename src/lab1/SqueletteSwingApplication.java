...
/**
 * <code>SqueletteSwingApplication</code> est un exemple d'une application en
 * Java qui fournit une interface Swing, avec un simple menu et un dessin.
 *
 * <h4>References</h4>
 * <ul>
 *
...
public class SqueletteSwingApplication extends JFrame implements ActionListener {
	 ...
	 *
	 */
	private Color theColor = new Color(0x000000); // 0xRRVVBB, rouge vert bleu
	// private Color theColor2 = new Color(0xFF0000); // 0xRRVVBB, rouge vert
	// bleu
	/*
	...
	private static final int NOMBRE_DE_FORMES = 150,
			DELAI_ENTRE_FORMES_MSEC = 500, DELAI_QUITTER_MSEC = 200;

	private static final int CANEVAS_LARGEUR = 500, CANEVAS_HAUTEUR = 500,
			MARGE_H = 50, MARGE_V = 60, FORME_MAX_LARGEUR = 200,
	 ...
	 */
	// private Ellipse2D.Double forme[] = new Ellipse2D.Double[10] ;
	private ArrayList<Object> ListFormes = new ArrayList<Object>();
	...
	public static void main(String[] args) {
		...
		ApplicationSupport.launch(frame,
				ApplicationSupport.getResource("app.frame.title"), 0, 0,
	...
	private JMenu makeFileMenu() {
		...
		JMenu fileMenu = ApplicationSupport.addMenu(this, MENU_FICHIER_TITRE,
	...
	private JMenu makeDrawMenu() {
		...
		// touche raccourci
		this.demarrerMenuItem.setAccelerator(KeyStroke.getKeyStroke(
		...
		// touche raccourci
		this.arreterMenuItem.setAccelerator(KeyStroke.getKeyStroke(
	...
	private JMenu makeHelpMenu() {
		JMenu helpMenu = ApplicationSupport.addMenu(this, MENU_AIDE_TITRE,
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (source.getText().equals(
				ApplicationSupport.getResource(MENU_DESSIN_DEMARRER))
				&& !this.workerActive) {
			/*
			 * Demander à l'usager combien de fois il veut que la forme change
			 */
			this.Server = new server();
			Server.connection("localhost", 10000);
			 ...
			 * plutôt qu'active, car il y a une forme definie dans le programme,
			 * mais elle va évoluer dans le temps, en se dessinant avec chaque
			 * changement. Ainsi, c'est une sorte d'animation qui prendra un
			 * certain temps.
			 * 
			 * Avec Java Swing, le Thread qui traite les appels à la méthode
			 * actionPerformed est le même qui répond à tous les évenements de
			 * la GUI. Si on lui donnait à faire ensuite un traitement long
			 * (l'animation), il ne pourrait plus répondre aux évenements. Pour
			 * éviter ce problème, on délègue le travail long à un thread
			 * special selon une stratégie proposée par Sun. C'est le
			 * SwingWorker. Pour plus d'informations sur cette approche, voir
			 * http
		...
		/*
		 * Traiter l'article de menu qui arrête le SwingWorker On met à false la
		 * valeur de l'attribut "workerActive" car la méthode dessinerFormes()
		 * va le tester dans sa boucle
		 */

		if (source.getText().equals(
				ApplicationSupport.getResource(MENU_DESSIN_ARRETER))
				&& this.workerActive) {
			this.workerActive = false;

			this.Server.close();
			this.ListFormes.removeAll(ListFormes);
			this.repaint();
		} else
		/*
		...
		if (source.getText().equals(
			...
			JOptionPane.showMessageDialog(this,
					ApplicationSupport.getResource(MESSAGE_DIALOGUE_A_PROPOS),
					ApplicationSupport.getResource(MENU_AIDE_PROPOS),
					JOptionPane.INFORMATION_MESSAGE);
		} else
		/*
		 * Traiter l'article "Quitter" ici On devrait arrêter le SwingWorker par
		 * principe, s'il est activé
		 */
		if (source.getText().equals(
				ApplicationSupport.getResource(MENU_FICHIER_QUITTER))) {
...
//			}
		 ...
		 * 
		 * 
		 * Formes input = new Formes(); input.fonctionforme(this.data.form,
		 * this.data.donne1, this.data.donne2, this.data.donne3,
		 * this.data.donne4 ); this.ListFormes.set(k,input); k++;
		 * 
		 * 
		 * 
		 ...
		 * faire le dessin. Le dessin se fait d'une manière indirecte -- ceci
		 * est une caractéristique de pratiquement tout système avec les
		 * fenêtres.
		 */
		// }*/
